...
The app expects the server to be running at `http://localhost:8000`. Once the backend is ready, the app's status indicator will show **Disconnected** (you can open the app now).
...
1. **Check the status** — Once the page loads, you should see a **Disconnected** status indicator (grey dot).
2. **Start Recording** — Click the **Start Recording** button to begin capturing audio from your microphone.
3. **Speak** — The app streams audio to the backend in real time. You should see the detected language label appear (e.g., "Language: Urdu").
...
- **No text appears while speaking:** The backend may not be ready. Refresh the page and wait for the status to show a green dot.
